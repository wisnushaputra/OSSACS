...
import { CreateOltInput } from '../olt.schema';
import { NotFoundError, ConflictError } from '../../../lib/errors';
import { pops, regions } from '../../../db/schema';

let oltRepository: OltRepository;
...
beforeEach(async () => {
  ...
  if (!region) throw new Error('Failed to seed Test Region for OLT Service');
  ...
  if (!pop) throw new Error('Failed to seed Test POP for OLT Service');
  testPopId = pop.id;
});
...
test('OltService: create OLT with duplicate name throws ConflictError', async () => {
  ...
  const duplicateOlt: CreateOltInput = {
...
test('OltService: update non-existent OLT throws NotFoundError', async () => {
  await assert.rejects(
    oltService.updateOlt('123e4567-e89b-12d3-a456-426614174000', { name: 'Non Existent' }),
    NotFoundError,
  );
});
