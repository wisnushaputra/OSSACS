...
import { CreateOltInput } from '../olt.schema'; // Use CreateOltInput
...
beforeEach(async () => {
  ...
  if (!region) throw new Error('Failed to seed Test Region');
  ...
  if (!pop) throw new Error('Failed to seed Test POP');
  testPopId = pop.id;
});
