...
import { onus, customers, olts, pops, regions } from '../../../db/schema'; // Import all necessary schemas
...
beforeEach(async () => {
  ...
  if (!region) throw new Error('Failed to seed ONU Test Region');
  ...
  if (!pop) throw new Error('Failed to seed ONU Test POP');
  testPopId = pop.id;
  ...
  if (!customer) throw new Error('Failed to seed Test Customer');
  ...
  if (!olt) throw new Error('Failed to seed Test OLT');
  testOltId = olt.id;
});
