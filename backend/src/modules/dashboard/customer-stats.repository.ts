...
export class CustomerStatsRepository {
  async getStatusSummary(filters: DashboardFilterParams) {
    ...
    if (startDate) conditions.push(gte(customers.createdAt, new Date(startDate)));
    if (endDate) conditions.push(lte(customers.createdAt, new Date(endDate)));
  ...
  async getByRegion(filters: DashboardFilterParams) {
    ...
    if (regionId) conditions.push(eq(regions.id, regionId));
    if (popId) conditions.push(eq(oltAlias.popId, popId));
    if (oltId) conditions.push(eq(onuAlias.oltId, oltId));
  ...
  async getByPop(filters: DashboardFilterParams) {
    ...
    if (regionId) conditions.push(eq(pops.regionId, regionId));
    if (popId) conditions.push(eq(pops.id, popId));
    if (oltId) conditions.push(eq(onuAlias.oltId, oltId));
