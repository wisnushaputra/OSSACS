...
import { NewCustomer } from '../customer.schema';
...
test('CustomerService: create customer with duplicate code throws ConflictError', async () => {
  ...
  const duplicateCustomer: NewCustomer = {
    customerCode: 'CUST001',
...
test('CustomerService: update non-existent customer throws NotFoundError', async () => {
  await assert.rejects(
    customerService.update('123e4567-e89b-12d3-a456-426614174000', { fullName: 'Non Existent' }),
    NotFoundError,
  );
});
