...
beforeEach(async () => {
    ...
    if (!region)
        throw new Error('Failed to seed ONU Service Test Region');
    ...
    if (!pop)
        throw new Error('Failed to seed ONU Service Test POP');
    testPopId = pop.id;
    // Seed Customer
    ...
    if (!customer)
        throw new Error('Failed to seed Test Customer for ONU Service');
    ...
    if (!olt)
        throw new Error('Failed to seed Test OLT for ONU Service');
    testOltId = olt.id;
});
after(async () => {
    await pool.end();
...
test('OnuService: update non-existent ONU throws NotFoundError', async () => {
    await assert.rejects(onuService.updateOnu('123e4567-e89b-12d3-a456-426614174000', { profileName: 'Non Existent' }), NotFoundError);
});
