...
import { users, roles, customers, olts, pops, regions } from '../../../db/schema';
...
const seedOnuDeps = async () => {
    ...
    if (!olt)
        throw new Error('Failed to seed olt');
    testOltId = olt.id;
};
before(async () => {
    server = createServer();
